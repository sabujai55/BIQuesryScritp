...
select 'PLC' as "BU"
	...
	, '' as "CancelDateTime"
	from order_item oi 
	inner join visit v on v.visit_id = oi.visit_id
	inner join item i on i.item_id = oi.item_id
	inner join employee e on oi.order_doctor_eid = e.employee_id 
	left join xray_result xr on oi.order_item_id = xr.order_item_id
	left join assign_xray ax on xr.assign_xray_id = ax.assign_xray_id
...
select 'PLC' as "BU"
	...
	inner join visit v on v.visit_id = toi.visit_id
	inner join item i on i.item_id = toi.item_id
	inner join employee e on toi.order_doctor_eid = e.employee_id 

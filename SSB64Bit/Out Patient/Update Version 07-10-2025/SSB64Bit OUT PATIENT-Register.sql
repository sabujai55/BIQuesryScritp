...
select 'PT2' as 'BU'
		...
		,vnp.CLINIC as 'ClinicCode' -- เปลี่ยนเป็น ClinicCode วันที่ 07/10/2568
		,dbo.sysconname(vnp.Clinic,42203,2) as 'ClinicNameTH' --แก้ไขวันที่ 26/02/268
		,dbo.sysconname(vnp.Clinic,42203,1) as 'ClinicNameEN' --เพิ่มวันที่ 26/02/2568
		,CAST(SUBSTRING(cndp.Com,27,13)as varchar) as 'ClinicDepartmentCode' --แก้ไขวันที่ 26/02/2568
		,dbo.sysconname(CAST(SUBSTRING(cndp.Com,27,13)as varchar),10145,2) as 'ClinicDepartmentNameTH' --แก้ไขวันที่ 26/02/2568
		,dbo.sysconname(CAST(SUBSTRING(cndp.Com,27,13)as varchar),10145,1) as 'ClinicDepartmentNameEN' --เพิ่มวันที่ 26/02/2568
		...
		, dbo.sysconname(doc.Clinic,42203,2) as DoctorClinicNameTH --เพิ่มวันที่ 07/10/2568
		, dbo.sysconname(doc.Clinic,42203,1) as DoctorClinicNameEN --เพิ่มวันที่ 07/10/2568
		, doc.ComposeDept as DoctorDepartmentCode
		, dbo.sysconname(doc.ComposeDept,10145,2) as DoctorDepartmentNameTH --เพิ่มวันที่ 07/10/2568
		, dbo.sysconname(doc.ComposeDept,10145,1) as DoctorDepartmentNameEN --เพิ่มวันที่ 07/10/2568
		, doc.Specialty as DoctorSpecialtyCode
		, dbo.sysconname(doc.Specialty,42197,2) as DoctorSpecialtyNameTH --เพิ่มวันที่ 07/10/2568
		, dbo.sysconname(doc.Specialty,42197,1) as DoctorSpecialtyNameEN --เพิ่มวันที่ 07/10/2568
		...
		,CAST(SUBSTRING(cndp.Com,27,13)as varchar) as 'ComposeDept' --เพิ่มวันที่ 26/02/2568
		...
		, vnp.PrivateCase --เพิ่มวันที่ 07/10/2568
		, vnm.InsuranceSalesAgent --เพิ่มวันที่ 07/10/2568
		, dbo.sysconname(vnm.InsuranceSalesAgent,43961,2) as InsuranceSalesAgentNameTH --เพิ่มวันที่ 07/10/2568
		, dbo.sysconname(vnm.InsuranceSalesAgent,43961,1) as InsuranceSalesAgentNameEN --เพิ่มวันที่ 07/10/2568

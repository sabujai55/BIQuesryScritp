select top 10 
		...
		,dbo.sysconname(apm.Clinic,42203,2) as ClinicNameTH --แก้ไขวันที่ 24/02/2568
		,dbo.sysconname(apm.Clinic,42203,1) as ClinicNameEN --เพิ่ม 24/02/2568
		,c.ComposeDept as ClinicDepartmentCode --แก้ไขวันที่ 19/03/2569
		,dbo.sysconname(c.ComposeDept,10145,2) as ClinicDepartmentNameTH --แก้ไขวันที่ 19/03/2569
		,dbo.sysconname(c.ComposeDept,10145,1) as ClinicDepartmentNameEN --แก้ไขวันที่ 19/03/2569
...
from	HNAPPMNT_HEADER apm
		...
		left join DEVDECRYPT.dbo.PYTS_SETUP_CLINIC_CODE c on apm.Clinic=c.Code --เพิ่ม 19/03/2569
